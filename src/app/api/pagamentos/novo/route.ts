...
export async function POST(request: Request) {
  try {
    ...
    await transporter.sendMail({
      ...
      subject: `✅ Novo Registro: ${dados.fornecedor}`,
      html: `
        <div style="font-family: sans-serif; border: 1px solid #eee; padding: 20px; border-radius: 8px;">
          ...
          <p>Um novo registro foi inserido no sistema:</p>
          ...
          <hr style="border: 1px solid #f4f4f4; margin: 15px 0;" />
          <p style="font-size: 12px; color: #888;">Este é um envio automático do Sistema ACE.</p>
        </div>
      `,
    });
