...
import RegrasClassificacaoModal from "./_components/RegrasClassificacaoModal";
...
export default function MovimentoPage() {
  ...
  const [modalAutoRegrasOpen, setModalAutoRegrasOpen] = useState(false);
  ...
  const contagemDuplicatas = useMemo(() => {
    ...
    const movsParaContar = movimentosFiltrados.filter(
      (m) => !m.ignorar_duplicidade,
    );
  ...
  const handleIgnorarDuplicidade = async (id: string) => {
    try {
      ...
      setMovimentos((prev) =>
        prev.map((m) =>
          m.id === id ? { ...m, ignorar_duplicidade: true } : m,
        ),
      );
    } catch (error) {
  ...
  const handleExcluirDuplicata = async (id: string, valor: number) => {
    if (
      window.confirm(
        `Tem certeza que deseja EXCLUIR DEFINITIVAMENTE este registro de R$ ${valor}?`,
      )
    ) {
      try {
  ...
  const movimentosRestantes = movimentosFiltrados.filter(
    (mov) => !checkIsDuplicada(mov),
  );

  const movimentosPendentes = movimentosRestantes.filter(
    (mov) => getStatusPreenchimento(mov) !== "completo",
  );
  const movimentosClassificados = movimentosRestantes.filter(
    (mov) => getStatusPreenchimento(mov) === "completo",
  );
  ...
  const renderCard = (mov: IMovimento) => {
    ...
    return (
      <MovimentoCard
        ...
        onIgnore={
          isDuplicada ? () => handleIgnorarDuplicidade(mov.id) : undefined
        }
        onDelete={
          isDuplicada
            ? () => handleExcluirDuplicata(mov.id, Number(mov.valor))
            : undefined
        }
  ...
  const exportarParaExcel = () => {
    ...
    const linhas = movimentosFiltrados.map((m) => {
      const dataFmt = m.data
        ...
        : "";
      const valorFmt = Number(m.valor || 0)
        .toFixed(2)
        .replace(".", ",");
      return [
        dataFmt,
  ...
  return (
    ...
    >
          ...
          {!isConsulta && (
            <>
              ...
              <Button
                ...
                startIcon={<AutoFixIcon />}
                sx={{ borderRadius: 2 }}
                onClick={() => setModalAutoRegrasOpen(true)}
              >
                <Typography
                  variant="body2"
                  sx={{ display: { xs: "none", sm: "block" } }}
          ...
          <>
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                {Object.entries(gruposDuplicados).map(([chave, grupoMovs]) => (
                  ...
                  >
                    ...
                    >
                      Identificados {grupoMovs.length} registros iguais (Banco,
                      Data e Valor)
                    </Typography>
            ...
            {movimentosPendentes.length > 0 && (
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                >
                  <WarningIcon /> Sem Classificação (
                  {movimentosPendentes.length})
                </Typography>
            ...
            {/* DIVISOR 2 */}
            {movimentosPendentes.length > 0 &&
              movimentosClassificados.length > 0 && <Divider sx={{ my: 1 }} />}
            ...
            {movimentosClassificados.length > 0 && (
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                >
                  <CheckCircleIcon /> Classificadas (
                  {movimentosClassificados.length})
                </Typography>
      ...
      />
      <RegrasClassificacaoModal
        open={modalAutoRegrasOpen}
        onClose={() => {
          setModalAutoRegrasOpen(false);
          setTriggerRefresh((prev) => prev + 1);
        }}
      />
    </Box>
